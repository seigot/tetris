...
ブロック操作用プログラムでは、ボード情報から次の動作を決定します。<br>
具体的には、[block_controller.py](https://github.com/seigot/tetris_game/blob/master/block_controller.py)の`def GetNextMove`が該当します。<br>
この関数では、`ボード情報`を入力として与えると、`次の動作決定用のデータ`を出力する仕様になっています。<br>
<br>
以下は、`ボード情報`、`次の動作決定用のデータ`について記載します。<br>
